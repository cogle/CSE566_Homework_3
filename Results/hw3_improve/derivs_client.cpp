...
void derivs_client() {
    ...
    MPI_Request mass_request;
    MPI_Request shield_request;
    ...
    MPI_Irecv(mass,npoints,MPI_DOUBLE,0,MPIDATA_PASSMASS,
        MPI_COMM_WORLD,&mass_request);
    MPI_Irecv(shield_rad,npoints,MPI_DOUBLE,0,MPIDATA_PASSSHIELD,
        MPI_COMM_WORLD,&shield_request);
    MPI_Irecv(x,npoints*6,MPI_DOUBLE,0,MPIDATA_PASSX,
        MPI_COMM_WORLD,&x_request);
    ...
    MPI_Wait(&mass_request, &g_mpi.status );
    MPI_Wait(&shield_request, &g_mpi.status );
    MPI_Wait(&x_request, &g_mpi.status );

    #pragma omp parallel shared(retval, block)
    {
